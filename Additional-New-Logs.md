...
```
##  Chat Event
This event will happen every time when the player open the "Chat" tool within Dani's tool panel. The value of the eventType could be "chatEvent". Specific variables under this eventType could be "chatID", "actionType" and "title". First of all, it will also contain whether the Chat session within the Objectives opens or closes, so the "actionType" under this situation could be "Open" and "Close"; Secondly, focusing on the content of the "Chat" tool's content, based on our discussions today, values of "actionType" should depend on dev team's work load. If it's not hard, then it could be more granular - "ScrollUp", "ScrollDown", "ScrollStop"; If it takes much time to finish, then it could be less granular - "Scroll" and "Stop". When the "actionType" is "Stop" or "ScrollStop", then the "ChatID" will contain a list of conversation plus dialogue ids, such as "[10-1, 10-2, 10-3]" (conversationId - dialogueNodeId) (if it's not a good data format, let me know). Example JSON record could be:
